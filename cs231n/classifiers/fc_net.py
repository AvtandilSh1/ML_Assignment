...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dout, dw, db = affine_backward(dscores, last_fc_cache)
        grads[f'W{self.num_layers}'] = dw + self.reg * self.params[f'W{self.num_layers}']
        grads[f'b{self.num_layers}'] = db

        for i in reversed(range(self.num_layers - 1)):
            cache_dict = layer_caches[i]
            if self.use_dropout:
                dout = dropout_backward(dout, cache_dict['dropout'])
            dout = relu_backward(dout, cache_dict['relu'])
            if self.normalization == 'batchnorm':
                dout, dgamma, dbeta = batchnorm_backward(dout, cache_dict['bn'])
                grads[f'gamma{i+1}'] = dgamma
                grads[f'beta{i+1}'] = dbeta
            elif self.normalization == 'layernorm':
                dout, dgamma, dbeta = layernorm_backward(dout, cache_dict['ln'])
                grads[f'gamma{i+1}'] = dgamma
                grads[f'beta{i+1}'] = dbeta
            dout, dw, db = affine_backward(dout, cache_dict['fc'])
